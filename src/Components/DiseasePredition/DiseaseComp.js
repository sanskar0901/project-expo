...
function DiseaseComp() {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const response = await axios.post(`${API_URI}//disease-predict`, formData, {
                headers: {
                    "Content-Type": "multipart/form-data",
                },
            });
            setDisease(response.data);
            console.log(diseaseData[disease.prediction].causes)
            setCauses(diseaseData[disease.prediction].causes)
            setPrevent(diseaseData[disease.prediction].prevent)
